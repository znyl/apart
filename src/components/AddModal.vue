...
export default {
    ...
    methods: {
        ...
        deleteInvoiceItem(_id) {
            this.invoiceItemList = this.invoiceItemList.filter(item => item.id !== _id)
        },
        callInvoiceTotal(){
            this.invoiceTotal = 0
            this.invoiceItemList.forEach(item => {
                this.invoiceTotal += item.qty * item.price
            })
        },
        closeInvoice() {
            this.TOGGLE_MODAL()
        ...
        async uploadInvoice() {
            if(this.invoiceItemList.length <= 0){
                alert('Please ensure you fill the items !')
                return
            } 

            this.callInvoiceTotal()

            const dataBase = db.collection('apartment').doc()
            await dataBase.set({
                invoiceId: uid(6),
